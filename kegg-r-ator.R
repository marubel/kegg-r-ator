...
# and ID cols
rename_select <- function(df) {
    sampid <- df$sample[1]
    df <- select(df, c("ko_gene_id", "weighted_kegg_pathways"))
    rename(df, !!sampid:=weighted_kegg_pathways)
}
...
agg_to_pathways=function(paths, out_path, matrix_path) {
    ...
    #p1 <- join_all(lapply(pathwaylist, rename_select), by="ko_gene_id", type = "full")
